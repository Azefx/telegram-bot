...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        await db.executescript('''
            CREATE TABLE IF NOT EXISTS codes (code TEXT PRIMARY KEY, used BOOLEAN DEFAULT 0);
        ''')
        await db.commit()
...
async def copy_address(call: CallbackQuery):
    ...
    method = call.data.upper().replace("_", " ")
    text = f"""
{decor(f'تم نسخ {method}')}
...
{PIN} بعد التحويل أرسل السكرين شوت + أيديك {ROCKET}
    ...
    await call.answer("✅ تم النسخ إلى الحافظة!")

# باقي الكود (الدفع بالنجوم + إضافة أكواد + admin)
...
async def successful_payment(msg: Message):
    if "month" in msg.successful_payment.invoice_payload:
        code = await get_available_code(msg.from_user.id)
        await bot.send_message(msg.from_user.id, f"{decor('تم الدفع!')} \n\nكودك: <code>{code}</code>" if code else "لا توجد أكواد")

@dp.message(F.photo)
async def handle_photo(msg: Message):
    await msg.answer(f"{decor('تم استلام السكرين شوت')}\n\n{PIN} أرسل أيديك الآن")
...
@dp.message()
async def handle_codes(msg: Message):
    if msg.from_user.id == ADMIN_ID:
